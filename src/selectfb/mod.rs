...
pub use self::value::{FbValue, Value, Number};
...
impl fmt::Debug for FbPathError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self)
    }
}

impl fmt::Display for FbPathError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            FbPathError::EmptyPath => f.write_str("path not set"),
            FbPathError::EmptyValue => f.write_str("json value not set"),
            FbPathError::Path(msg) => f.write_str(&format!("path error: \n{}\n", msg)),
            FbPathError::FlexBuffers(msg) => f.write_str(&format!("serde error: \n{}\n", msg)),
        }
    }
}
...
impl<'a> FilterTerms<'a> {
    ...
    fn filter_json_term<F: Fn(&Vec<FbValue<'a>>, &mut Vec<FbValue<'a>>, &mut HashSet<usize>) -> FilterKey>(
        &mut self,
        e: ExprTerm<'a>,
        fun: F,
    ) {
        ...
        if let ExprTerm::Json(rel, fk, vec) = e {
            ...
            let filter_key = if let Some(FilterKey::String(key)) = fk {
                let key_contained = &vec.iter().map(|v| match v.clone().into() {
            ...
            if rel.is_some() {
                ...
                let filtered: Vec<FbValue> = vec.iter().enumerate()
                    .filter(
    ...
    fn push_json_term<F: Fn(&Vec<FbValue<'a>>, &mut Vec<FbValue<'a>>, &mut HashSet<usize>) -> FilterKey>(
        &mut self,
        current: &Option<Vec<FbValue<'a>>>,
        fun: F,
    ) {
    ...
    fn filter<F: Fn(&Vec<FbValue<'a>>, &mut Vec<FbValue<'a>>, &mut HashSet<usize>) -> FilterKey>(
        &mut self,
        current: &Option<Vec<FbValue<'a>>>,
        fun: F,
    ) {
    ...
    fn filter_all_with_str(&mut self, current: &Option<Vec<FbValue<'a>>>, key: &str) {
    ...
    fn filter_next_with_str(&mut self, current: &Option<Vec<FbValue<'a>>>, key: &str) {
    ...
    fn collect_next_with_num(&mut self, current: &Option<Vec<FbValue<'a>>>, index: f64) -> Option<Vec<FbValue<'a>>> {
        fn _collect<'a>(tmp: &mut Vec<FbValue<'a>>, vec: &[FbValue<'a>], index: f64) {
    ...
    fn collect_next_all(&mut self, current: &Option<Vec<FbValue<'a>>>) -> Option<Vec<FbValue<'a>>> {
        if let Some(current) = current {
            let mut tmp = Vec::new();
            for c in current {
                match c.clone().into() {
    ...
    fn collect_next_with_str(&mut self, current: &Option<Vec<FbValue<'a>>>, keys: &[String]) -> Option<Vec<FbValue<'a>>> {
        if let Some(current) = current {
            let mut tmp = Vec::new();
            for c in current {
                if let Value::Object(map) = c.clone().into() {
                    for key in keys {
    ...
    fn collect_all(&mut self, current: &Option<Vec<FbValue<'a>>>) -> Option<Vec<FbValue<'a>>> {
        if let Some(current) = current {
            let mut tmp = Vec::new();
    ...
    fn collect_all_with_str(&mut self, current: &Option<Vec<FbValue<'a>>>, key: &str) -> Option<Vec<FbValue<'a>>> {
        if let Some(current) = current {
            let mut tmp = Vec::new();
    ...
    fn collect_all_with_num(&mut self, current: &Option<Vec<FbValue<'a>>>, index: f64) -> Option<Vec<FbValue<'a>>> {
        if let Some(current) = current {
            let mut tmp = Vec::new();
...
impl<'a, 'b> Selector<'a, 'b> {
    ...
    pub fn str_path(&mut self, path: &str) -> Result<&mut Self, FbPathError> {
        debug!("path : {}", path);
        self.node_ref.take();
        self.node = Some(Parser::compile(path).map_err(FbPathError::Path)?);
        Ok(self)
    }
    ...
    pub fn value(&mut self, v: FbValue<'a>) -> &mut Self {
        self.value = Some(v);
        self
    }

    fn _select(&mut self) -> Result<(), FbPathError> {
        ...
        if self.node.is_none() {
            return Err(FbPathError::EmptyPath);
        }
    ...
    pub fn select_as<T: serde::de::DeserializeOwned>(&mut self) -> Result<Vec<T>, FbPathError> {
        self._select()?;
        ...
        match &self.current {
            Some(vec) => {
                let mut ret = Vec::new();
                for v in vec {
                    match T::deserialize(v.reader()) {
                        Ok(v) => ret.push(v),
                        Err(e) => return Err(FbPathError::FlexBuffers(e.to_string())),
                    }
                }
                Ok(ret)
            }
            _ => Err(FbPathError::EmptyValue),
        }
    }
//
//     pub fn select_as_str(&mut self) -> Result<String, FbPathError> {
//         self._select()?;
//
//         match &self.current {
//             Some(r) => {
//                 Ok(serde_json::to_string(r).map_err(|e| FbPathError::Serde(e.to_string()))?)
//             }
//             _ => Err(FbPathError::EmptyValue),
//         }
//     }

    pub fn select(&mut self) -> Result<Vec<FbValue<'a>>, FbPathError> {
        self._select()?;
    ...
    fn visit_absolute(&mut self) {
        if self.current.is_some() {
            ...
            if let Some(value) = self.value.clone() {
                selector.value = Some(value.clone());
    ...
    fn visit_range(&mut self, from: &Option<isize>, to: &Option<isize>, step: &Option<usize>) {
        ...
        if let Some(ParseToken::Array) = self.tokens.pop() {
            ...
            if let Some(current) = &self.current {
                for v in current {
                    if let Value::Array(vec) = v.clone().into() {
...
//
//     pub fn str_path(&mut self, path: &str) -> Result<&mut Self, FbPathError> {
//         self.path = Some(Parser::compile(path).map_err(FbPathError::Path)?);
//         Ok(self)
//     }
//
...
//     }
//
//     pub fn delete(&mut self) -> Result<&mut Self, FbPathError> {
//         self.replace_with(&mut |_| Some(Value::Null))
//     }
//
//     pub fn remove(&mut self) -> Result<&mut Self, FbPathError> {
//         self.replace_with(&mut |_| None)
//     }
//
//     fn select(&self) -> Result<Vec<&Value>, FbPathError> {
