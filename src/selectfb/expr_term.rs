...
use selectfb::value::{FbValue, Value, Number};
use selectfb::cmp::*;
use selectfb::{FilterKey, to_f64};

#[derive(Debug, PartialEq)]
pub(super) enum ExprTerm<'de> {
    String(String),
    Number(Number<'de>),
    Bool(bool),
    Json(Option<Vec<FbValue<'de>>>, Option<FilterKey>, Vec<FbValue<'de>>),
}
...
impl<'de> ExprTerm<'de> {
    fn cmp<C1: Cmp, C2: Cmp>(
        ...
        match &self {
            ...
            ExprTerm::Json(rel, fk1, vec1) => {
                let ret: Vec<FbValue<'de>> = match &other {
                    ExprTerm::String(s2) => vec1
                        .iter()
                        .filter(|v1| match (*v1).clone().into() {
                            Value::String(s1) => cmp_fn.cmp_string(&s1, s2),
                            Value::Object(map1) => {
                                if let Some(FilterKey::String(k)) = fk1 {
                                    if let Some(Value::String(s1)) = map1.get(k).map(Into::into) {
                    ...
                    ExprTerm::Number(n2) => vec1
                        ...
                        .filter(|v1| match (*v1).clone().into() {
                            ...
                            Value::Object(map1) => {
                                if let Some(FilterKey::String(k)) = fk1 {
                                    if let Some(Value::Number(n1)) = map1.get(k).map(Into::into) {
                    ...
                    ExprTerm::Bool(b2) => vec1
                        ...
                        .filter(|v1| match (*v1).clone().into() {
                            ...
                            Value::Object(map1) => {
                                if let Some(FilterKey::String(k)) = fk1 {
                                    if let Some(Value::Bool(b1)) = map1.get(k).map(Into::into) {
    ...
    pub fn eq(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
        debug!("eq - {:?} : {:?}", &self, &other);
        let _ = ret.take();
    ...
    pub fn ne(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
        debug!("ne - {:?} : {:?}", &self, &other);
        let _ = ret.take();
    ...
    pub fn gt(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
        debug!("gt - {:?} : {:?}", &self, &other);
        let _ = ret.take();
    ...
    pub fn ge(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
        debug!("ge - {:?} : {:?}", &self, &other);
        let _ = ret.take();
    ...
    pub fn lt(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
        debug!("lt - {:?} : {:?}", &self, &other);
        let _ = ret.take();
    ...
    pub fn le(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
        debug!("le - {:?} : {:?}", &self, &other);
        let _ = ret.take();
    ...
    pub fn and(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
    ...
    pub fn or(&self, other: &Self, ret: &mut Option<ExprTerm<'de>>) {
        debug!("or - {:?} : {:?}", &self, &other);
        let _ = ret.take();
...
impl<'de> Into<ExprTerm<'de>> for &Vec<FbValue<'de>> {
    fn into(self) -> ExprTerm<'de> {
        if self.len() == 1 {
            match &self[0].clone().into() {
